...
class TestTupleSource : public buzzdb::operators::Operator {
 ...
 public:
  ...
  explicit TestTupleSource(const std::vector<std::tuple<Ts...>>& tuples)
      : tuples(tuples) {}

  void open() override {
    output_registers.resize(sizeof...(Ts));
    opened = true;
  }

  bool next() override {
    if (curr_idx < tuples.size()) {
      write_to_registers(output_registers, tuples[curr_idx]);
      ++curr_idx;
      return true;
    } else {
      return false;
